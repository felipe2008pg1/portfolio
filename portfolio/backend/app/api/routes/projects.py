...
def list_all_projects_admin(db: Session = Depends(get_db), _admin: str = Depends(get_current_admin)):
    return project_service.list_all_projects(db)


@router.post("", response_model=ProjectOut, status_code=status.HTTP_201_CREATED)
def create_project(
    payload: ProjectCreate, db: Session = Depends(get_db), _admin: str = Depends(get_current_admin)
):
...
def delete_project(
    project_id: int, db: Session = Depends(get_db), _admin: str = Depends(get_current_admin)
):
    project = project_service.get_project(db, project_id)
    if project is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Projeto não encontrado.")
