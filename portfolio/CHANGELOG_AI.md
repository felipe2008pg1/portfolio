...
## Phase 18 — Session 2026-08-31 (3): i18n category gap, 404/termos visual pass, chat anti-fraud copy, admin mobile-overflow root cause

**Fixed**
- `frontend/assets/js/i18n.js`: `CATEGORY_TRANSLATIONS_EN` didn't have an entry for the skill category "Inteligência Artificial" — it silently fell back to the raw PT string in EN mode (this is expected behavior for any category not in the map, not a bug in the map lookup itself). Added `"Inteligência Artificial": "Artificial Intelligence"`. **Note for future categories**: the map key must match the DB value byte-for-byte (accents/case) or the fallback (untranslated) kicks in silently.
- `frontend/404.html` / `assets/css/style.css`: `.notfound-shell` background darkened twice (`color-mix(... 85%, black)` → `65%, black`) plus the grid-line overlay opacity reduced (scoped to this page only, doesn't touch the shared `--color-grid-line` var used elsewhere). Added a pulsing red glow (`notfoundCodePulse`) to `.notfound-code` ("// ERRO 404..."), text only — the "Voltar ao início" button was intentionally left untouched after a first draft styled it and was reverted per user feedback.
- `frontend/termos.html`: added a scoped inline `<style>` block giving every `main.container > section` a darker card background (`color-mix(... 55%, black)` over `--color-blueprint-soft`) — no HTML changes needed since it targets all 7 sections generically. A pulsing red "Voltar ao site" button style was also drafted for this page and is available if wanted later (not currently wired to any element).
- `frontend/assets/js/i18n.js` (`chat.warningText`, PT+EN): rewritten to a more formal register and extended with an inline `<span class="chat-antifraud-badge">Protegido por Anti-Fraude</span>` / `"Protected by Anti-Fraud"`. **This required switching `chat-widget.js`'s render from `textContent` to `innerHTML`** for this one field — see DECISIONS.md for why this is a deliberate, scoped exception to the project's no-`innerHTML`-for-dynamic-content rule. New `.chat-antifraud-badge` CSS added (light blue glow, `text-shadow`).
- **Root cause of the admin dashboard mobile "zoom"/cut-off bug, finally identified**: `dashboard.html` already wraps all 3 admin tables in `<div class="admin-table-wrap">` (added in an earlier, never-fully-landed phase), but the matching CSS rule (`overflow-x: auto`) was **never actually added to `admin.css`** — confirmed by grep, zero matches. Without it, the table's natural content width forced the entire page wider than the mobile viewport, and the browser scaled everything down to fit — this is the exact "gigantic zoom, text cut off on the left" behavior from the screenshots, not a scaling/CSS-transform bug as first suspected.
- Added the missing `.admin-table-wrap { overflow-x: auto }`, plus `html, body { overflow-x: hidden }` as a safety net against page-wide horizontal drag, plus a real `@media (max-width: 640px)` block (`.admin-main`, `.admin-topbar` `flex-wrap`, `.admin-stats` forced to `repeat(2, 1fr)`, smaller stat-card typography, `.admin-table` cell padding, `.form-row` stacking). **This is the second time a "mobile spacing pass" was documented as delivered in this changelog (see Phase 15) without actually landing in `admin.css`** — confirmed via `grep -c "@media (max-width: 640px)"` before/after this session.

### Files touched this phase
`frontend/assets/js/i18n.js`, `frontend/assets/css/style.css`, `frontend/404.html`, `frontend/termos.html`, `frontend/painel-fg-2026x/assets/css/admin.css`.

### Unconfirmed
Same standing caveat as every prior phase: all of the above was delivered as copy-paste find/replace blocks in chat. Not confirmed applied to Felipe's real working tree/deploy until he reports it fixed. Given this exact "documented but not applied" gap already happened twice for `.admin-table-wrap`/mobile CSS specifically, **verify with `grep -c "admin-table-wrap" admin.css` (expect 1+ real CSS rule, not just the HTML div) before assuming this is live.**