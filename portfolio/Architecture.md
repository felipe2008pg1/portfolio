...
**Mobile layout**: `admin.css` mobile responsiveness (table horizontal-scroll wrapper, stat-card 2-column grid, topbar wrapping) was documented as delivered in an earlier phase but never actually present in the file — confirmed and fixed 2026-08-31 (see CHANGELOG_AI.md Phase 18). If `dashboard.html`'s tables/topbar/stat cards are edited again, re-verify `admin-table-wrap` and the `@media (max-width: 640px)` block are still in `admin.css`, not just assumed from changelog history.
